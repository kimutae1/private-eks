#!/bin/bash
BITBUCKET_SERVER="https://code.bespinglobal.com"
...
project_list=$(curl -s -u "${BITBUCKET_USERNAME}:${BITBUCKET_PASSWORD}" "${BITBUCKET_SERVER}/rest/api/1.0/projects" | jq -r '.values[] | .name + "|" + .key')
...
repo_list=$(curl -s -u "${BITBUCKET_USERNAME}:${BITBUCKET_PASSWORD}" "${BITBUCKET_SERVER}/rest/api/1.0/projects/${project_key}/repos" | jq -r '.values[] | .slug')
...
select repo_slug in $repo_list "모든 리포지토리 클론"; do
    if [[ $repo_slug == "모든 리포지토리 클론" ]]; then
        ...
        for repo in $repo_list; do
            git clone "${BITBUCKET_SERVER}/scm/${project_key}/${repo}.git"
        done
        break
    else
...
case $action in
    clone)
        echo "저장소를 클론하는 중..."
        git clone "${BITBUCKET_SERVER}/scm/${project_key}/${repo_slug}.git"
        ;;
    save)
